...
public class FieldOrientedDrive extends Command {
  ...
  public FieldOrientedDrive(CommandSwerveDrivetrain driveTrain, Linkage linkage) {
  ...
  @Override
  public void execute() {
    if(linkage.getAngle() < 90.0) {
      driveTrain.fieldCentricDrive(
        .5*UtilMethods.squareInput(MathUtil.applyDeadband(-driverController.getLeftX(), 0.1)),
        .5*UtilMethods.squareInput(MathUtil.applyDeadband(-driverController.getLeftY(), 0.1)),
        .5*UtilMethods.squareInput(MathUtil.applyDeadband(driverController.getRightX(), 0.1))
    );
    }
    else {
    driveTrain.fieldCentricDrive(
